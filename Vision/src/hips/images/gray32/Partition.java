...
public class Partition extends hips.Partition<ImageGray32, PixelValue> {
    ...
    public Partition(ImageGray32 input, PixelValue alpha, PixelValue omega, float ci) {
            ...
            if (lbl[p] == -1) {
                ...
                while (!PQueue.isEmpty()) {
                    ...
                    if (datum.getPriority() > rcrt) {
                        ...
                        while (stack.hasNew()) {
                            ...
                            for (int z = 0; z < Neighborhood.NEIGHBORS; z++) {
                                ...
                                if (connected[q]) {
                                    edges++;
                                    if (f.getPixelValue(r).range(f.getPixelValue(q)).maxValueFloat() <= alphaFloat) {
                                        edgesOK++;
                                    }
                                }
                            }
                    ...
                    for (int z = 0; z < Neighborhood.NEIGHBORS; z++) {
                        ...
                        float rlval = (f.getPixelValue(q).range(f.getPixelValue(datum.getPixel()))).maxValueFloat();
