...
public class Partition extends hips.Partition<ImageRGB, PixelValue> {

	public Partition(ImageRGB input, PixelValue alpha, PixelValue omega,
			float ci) {
		super(input, alpha, omega, ci);
	}

	public Partition(ImageRGB input) {
		super(input);
	}
	...
	public void makeRegions() {
		...
		ImageRGB f = (ImageRGB) returned[1];
		PixelValue omegaTr = (PixelValue) returned[2];

		System.out.println("ALPHAINT: " + alphaInt);
		System.out.println("OMEGATRB: " + omegaTr.getString());
			...
			if (lbl[p] == -1) {
				...
				while (!PQueue.isEmpty()) {
					...
					for (int z = 0; z < Neighborhood.NEIGHBORS; z++) {
						...
						int rlval = (f.getPixelValue(q).range(f
...
//		}
//		ImageRGB working = (ImageRGB) getWorkingImage(image, transforming);
//		returned[1] = working;
//		returned[2] = omg;
//		return returned;
//	}
